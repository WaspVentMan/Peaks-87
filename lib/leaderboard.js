...
function renderLB(){
    ...
    if (leaderboard == "z"){
        ...
        for (let x = 0; x < 9; x++){
            boardRender.push("_" + (x+1) + "," + scores[x][0] + renderDots(32-("_0,AAA" + scores[x][1] + "m").length) + scores[x][1] + "m")
        }

        boardRender.push("10," + scores[9][0] + renderDots(32-("_0,AAA" + scores[9][1] + "m").length) + scores[9][1] + "m")
        boardRender.push("")
        ...
        if (!offline){
            ...
            NGIO.getScores(deathboard, options, function(scores, board, options){
                ...
                for (let x = 0; x < 9; x++){
                    boardRender.push("_" + (x+1) + "," + scores[x].user.name + renderDots(32-("_0," + scores[x].user.name + (scores[x].value/100) + "m").length) + (scores[x].value/100) + "m")
                }

                boardRender.push("10," + scores[x].user.name + renderDots(32-("_0," + scores[9].user.name + (scores[9].value/100) + "m").length) + (scores[9].value/100) + "m")
                boardRender.push("")
                boardRender.push("z_for_local___x_for_global")
                boardRender.push("")
                boardRender.push("z_and_x_to_restart")
