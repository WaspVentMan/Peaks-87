...
function renderChar(char, font="default"){
    ...
    if (char == "*"){return `<div style="width: 16px; height: 16px; background-image: url(img/local.png);"></div>`}
    if (char == "@"){return `<div style="width: 16px; height: 16px; background-image: url(img/global.png);"></div>`}
    let letter = [false, 0]
    for (let x = 0; x < charIndex.length; x++){if (char == charIndex[x]){letter = [true, x]; break}}
    if (letter[0]){return `<div style="width: 8px; height: 16px; background-image: url(img/font/${font}.png); background-position: ${(letter[1]+1)*8}px 0px"></div>`}
...
function renderString(string, font="default", allign="centre"){
    ...
    if (string == "£"){
        ...
        return `<div style="width: 48px; height: 64px; margin: auto; background-image: ` + skin + `;"></div>`
    }
