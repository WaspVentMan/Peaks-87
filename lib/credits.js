...
function playCredits(title = "none", board = false){
    ...
    let creditsMusic = new Audio("audio/lanette.mp3", 15)
    ...
    let creditslife = setInterval(function(){
        ...
        if (!mobile){
            ...
            creditsScroll = (creditsText.getBoundingClientRect().height/settings.zoom)*(((Date.now()-startTime)/1000) / 131.291428)
        }
        ...
        if ((creditsMusic.currentTime / creditsMusic.duration) >= 1 && !mobile || (((Date.now()-startTime)/1000) / 131.291428) >= 1 && mobile){
